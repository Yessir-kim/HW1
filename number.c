...
void printSubset(int subSet[], int size, int subAdd[], int index, bool right = true) {
  if(right)
  {
    if(size == 1)
    {
...
void subsetSum(int set[], int subSet[], int n, int subSize, int total, int count ,int sum, int subAdd[], int index, bool right = true) {
    if(total == sum) { // corrext case
       printSubset(subSet, subSize, subAdd, index, right);     // print the subset
       return;
    }
      ...
      for(int i = count; i < n; i++ ) {
        ...
        subAdd[subSize] = i;
        subsetSum(set, subSet, n, subSize+1, total+set[i], i+1, sum, subAdd, index, right);     // search next subset
       }
    }
}

void findSubset(int set[], int size, int sum, int index, bool right = true) {
    ...
    subsetSum(set, subSet, size, 0, 0, 0, sum, subAdd, index, right);
    free(subSet);
}
...
/*for(int i = 0; i < 9; i++){
  ...
  for (int i = 0; i < row; i++)
  {
    ...
    findSubset(temp_row, row, label_row[i], i+1);
    fprintf(fp,"))\n");
  }
  free(temp_row);
  free(label_row);
  ...
  for (int i = 0; i < col; i++)
  {
    ...
    //printf("\n");
    fprintf(fp,"(assert(or ");
    findSubset(temp_col, col, label_col[i], i+1, false);
    fprintf(fp,"))\n");
  }
  free(temp_col);
  free(label_col);
